...
#            self.key = "q"
            ...
            white_balance_node = PySpin.CEnumerationPtr(nodemap.GetNode('BalanceWhiteAuto'))
            white_balance_off = white_balance_node.GetEntryByName('Off')
            off_white = white_balance_off.GetValue()
            white_balance_node.SetIntValue(off_white)
            
            balance_node = PySpin.CEnumerationPtr(nodemap.GetNode('BalanceRatioSelector'))
            balance_red = balance_node.GetEntryByName('Red')
            red_time = balance_red.GetValue()
            balance_node.SetIntValue(red_time)
            
            ratio_node = PySpin.CFloatPtr(nodemap.GetNode('BalanceRatio'))
            ratio_node.SetValue(1.05)

            balance_node = PySpin.CEnumerationPtr(nodemap.GetNode('BalanceRatioSelector'))
            balance_red = balance_node.GetEntryByName('Blue')
            red_time = balance_red.GetValue()
            balance_node.SetIntValue(red_time)
            
            ratio_node = PySpin.CFloatPtr(nodemap.GetNode('BalanceRatio'))
            ratio_node.SetValue(4)

            camera.GammaEnable.SetValue(False)

            Gain_node = PySpin.CEnumerationPtr(nodemap.GetNode('GainAuto'))
            Gain_off = Gain_node.GetEntryByName('Off')
            off_Gain = Gain_off.GetValue()
            Gain_node.SetIntValue(off_Gain)
